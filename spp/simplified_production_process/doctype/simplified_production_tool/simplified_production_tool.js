...
frappe.ui.form.on("Simplified Production Tool", {
  ...
  manufacture_items: function (frm) {
    frappe.call({
      doc: frm.doc,
...
cur_frm.cscript.item_code = function (doc, cdt, cdn) {
  ...
  if (d.item_code) {
    frappe.call({
      method:
        "erpnext.manufacturing.doctype.work_order.work_order.get_item_details",
      args: {
        item: d.item_code,
      },
      callback: function (r) {
...
cur_frm.cscript.download_materials_required = function (doc, cdt, cdn) {
  return $c_obj(doc, "validate_data", "", function (r, rt) {
    if (!r["exc"]) $c_obj_csv(doc, "download_raw_materials", "", "");
  });
};

cur_frm.fields_dict["sales_orders"].grid.get_field("sales_order").get_query =
  function (doc) {
...
cur_frm.fields_dict["items"].grid.get_field("bom_no").get_query = function (
  doc,
  cdt,
  cdn
) {
